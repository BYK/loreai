...
| `memoryPreparationTimeoutMs` | number | — | min 1000, max 60000 | Post-decode memory preparation budget in milliseconds. Default: 8000. LORE_MEMORY_PREPARATION_TIMEOUT_MS takes precedence at request admission. Expiry returns a retryable preparation error before forwarding an unsafe prompt. |
...
| `inFlight` | boolean | `false` |  | Run the curator mid-conversation (turn-based), not just on idle. Default: false. WARNING: only enable on free-write / non-caching providers (e.g. MiniMax). On cache-sensitive providers (Anthropic), mid-session curation changes the knowledge base, which queues a knowledge-delta block (user→assistant pair appended mid-conversation) that re-writes part of the prompt tail each time the knowledge set changes. Deferring curation to idle lets the queue stay empty during active turns. Where cache writes are free this is harmless and yields fresher knowledge sooner. |

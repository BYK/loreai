...
| `memoryPreparationTimeoutMs` | number | — | min 1000, max 60000 | Post-decode memory preparation budget in milliseconds. Default: 8000. LORE_MEMORY_PREPARATION_TIMEOUT_MS takes precedence at request admission. A verified accepted source window and knowledge pin may continue on optional refresh expiry; otherwise preparation returns a retryable error. |

...
| `LORE_BEDROCK_REGION` | Resolve the default AWS Bedrock region from the standard env chain (`LORE_BEDROCK_REGION` → `AWS_REGION` → `AWS_DEFAULT_REGION` → "us-east-1"). Shared between `loadConfig()` and call sites that build a Bedrock profile without a threaded config (e.g. the cache-warmer dashboard snapshot), so they never diverge on a hard-coded region. |

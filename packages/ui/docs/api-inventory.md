...
|---|---|---|---|
| `GET /api/v1/projects` | `ProjectSummary[]`: `id, path, name, git_remote, created_at, knowledge_count, session_count, message_count, distillation_count`, newest first. All `*_at` timestamps in this API are epoch **milliseconds** (integers), never ISO strings. `knowledge_count` counts current entries with `confidence > 0.2`. | `core/data.listProjects` | No filter, no limit, no search. |

 ...
 * (and hash) the final binary, avoiding the redundant disk write, temp-copy,
 * and SHA-256 pass that a file-by-file chain would incur per hop. The running
 * binary is the only input copied to a temp file. See
 * {@link applyPatchChainInMemory}.
 *
 ...
 * metadata-only reflink (near-instant). Reading on demand keeps the ~310 MB
 ...
 * cost on a ~310 MB binary. We instead process 4 bytes per iteration with a
 ...
 * verified exhaustively. A short tail loop handles the trailing `n % 4` bytes.
 ...
 *
 * Applies a patch to base bytes, emitting output chunks in order via `onChunk`.
 * Handles header parsing, streaming zstd decompression of the diff and extra
 * blocks, the wrapping-add reconstruction, and output-size validation.
 *
 ...
 * base for the next patch without ever touching disk, and no SHA-256 is
 * computed (only the final binary is hashed and verified).
 */

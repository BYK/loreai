...
acquire_lifecycle_lock
if [[ -n "${LORE_INSTALL_OBSERVED_TOMBSTONE:-}" ]]; then
  bootstrap_marker=missing
  if [[ -f "$uninstall_tombstone_path" ]]; then bootstrap_marker=$(hash_file "$uninstall_tombstone_path"); fi
  [[ "$bootstrap_marker" == "$LORE_INSTALL_OBSERVED_TOMBSTONE" ]] || die "Uninstall generation changed while downloading; retry the installer"
fi

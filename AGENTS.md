...
| Gateway server | `packages/gateway/src/server.ts` (node:http bridge + lifecycle), `packages/gateway/src/app.ts` (Hono routes + access middleware), `packages/gateway/src/management-access.ts` (access policy helpers) |

...
* **normalize erases operators/punctuation**: Trap: stripping ALL non-alphanumeric chars collapses \`>= floor\` and \`> floor\` to \`floor\`, \`x == y\` and \`x != y\` to \`x y\`, etc. — suppressing material edits in code/config-heavy entries. Fix: replace the 'strip everything non-alphanumeric' with 'strip a specific set of cosmetic characters, collapse whitespace.'

...
<!-- lore:019f84cd-f366-7799-80cb-8bae5fc4e19e -->
* **markWorkerPaused credit-pause must never trip the Sentry outage ladder**: Worker-health invariant in the loreai gateway: credit pauses via markWorkerPaused must never open the failure circuit or escalate to Sentry — a credit pause is an expected/benign state (session out of budget), not a worker malfunction. Keep this distinct from the "data-policy" failure reason (also excluded from the Sentry ladder) and from true worker degradation (3 truncations in 5 min → DEGRADED). These failure classes must stay separately gated so benign/expected states never pollute the outage-escalation signal used to alert on real worker health problems.
...
<!-- lore:019f84cd-f30f-7fbb-b72e-7508c7b12020 -->
* **Worker retry-loop reasoning-headroom floor: apply once at loop entry, not builder+retry**: llm-adapter.ts's worker retry loop (maxTokens initialized ~line 1776) must apply the reasoning-headroom floor (16384 = 8192 base + 8192 headroom) ONCE at loop entry, gated on protocol (openai/gemini) and thinking-on-by-default detection — never separately in the request builder AND the retry bump. Applying it in both places causes a raw-vs-floored mismatch: retry's maxTokens\*RETRY\_MULTIPLIER computed off the unfloored value can land BELOW what the builder already floored and sent, shrinking the retry budget. Once centralized, the retry bump is just maxTokens \* WORKER\_LENGTH\_RETRY\_MULTIPLIER. Retry guard is min(bumpedCandidate, ceiling) > maxTokens (not maxTokens < ceiling), so retry only fires if it can actually raise the effective budget. Floor is NOT applied to Anthropic/Vertex builders — those suppress thinking via thinking:{type:"disabled"}, so no reasoning burn occurs there.
...
<!-- lore:019f84e0-71c2-7c5b-8b81-5d95135fede8 -->
* **anthropic-client-openai-upstream-stream.test.ts flakes under full-suite parallel load**: The test 'anthropic-client-openai-upstream-stream.test.ts: re-emits a buffered OpenAI upstream as an Anthropic SSE stream' spawns a real server (startServer/loadConfig), resets DB (closeDB/resetPipelineState/loadLoreConfig), and sends a real fetch POST — it's sensitive to port binding/DB timing when run in parallel with the rest of the suite (pnpm test, 6654 tests). Trap: a failure here during a full-suite run looks like a regression from whatever you just changed, since it's the only failing test. Fix: re-run the single file in isolation first — if it passes standalone, it's this pre-existing flake, not your change. Confirmed unrelated to reasoning-budget/config constant edits.
...
<!-- lore:019f84cd-f2e5-797b-b7dc-fad157d79a3a -->
* **buildOpenAIWorkerRequest lacked reasoning headroom vs Anthropic/Vertex paths**: In llm-adapter.ts, buildOpenAIWorkerRequest() sent max\_completion\_tokens: maxTokens with no reasoning headroom, unlike the Anthropic path (Math.max(maxTokens, thinkingBudget+THINKING\_OUTPUT\_HEADROOM)) and the Vertex path (same pattern). This gap was latent while OpenRouter workers used cheap non-reasoning models; PR #1407's lineage-aware selection then routed sessions to anthropic/claude-sonnet-5 (a true reasoning model) via the OpenAI protocol, newly exposing it as 'worker empty response, finish\_reason=length'. Fixed in PR #1413: align all three builders to include reasoning headroom, plus a provider-agnostic retry-on-length backstop (max 1 retry, rebuilds request via buildWorkerRequest so the billing signature recomputes, gated on maxTokens < workerLengthRetryCeiling(model)).
...
<!-- lore:019f84cd-f33b-74ec-a408-f360c1d6fea8 -->
* **OpenRouter :free models zeroed cost always won worker selection; 404 needs hard classification**: worker-model.ts zeroed the cost of OpenRouter :free model variants so they always won 'cheapest' worker selection — but OpenRouter's data-policy guardrails reject them with a 404 'No endpoints available matching your guardrail restrictions and data policy' once training/logging opt-out is required. llm-adapter.ts originally classified this 404 as retriable 'upstream-error', causing a retry storm instead of recognizing it as a hard worker-incapable fact. Fix (PR #1407): don't statically exclude :free variants — learn incapability only after an actual data-policy 404 is observed, tag it with a new "data-policy" failure reason excluded from the Sentry outage-escalation ladder. Never opt into OpenRouter's data policy just to unlock :free endpoints (permits prompt logging/training) — prefer pinning a paid model instead.
...
<!-- lore:019f84a1-fa2a-7379-9144-02bc9f7b5d4e -->
* **PR #1426: worker reasoning budget floor (16384) + effort-typed model gotcha**: Distill/curation worker's first-attempt reasoning budget was floored too low (2048/4096), causing frequent finish\_reason=length truncation requiring retries; fixed by flooring the first attempt at 16384 via workerReasoningHeadroomFloor. Gotcha: the floor was gated on workerThinkingOnByDefault (true only when reasoning\_options.type==='toggle'), so effort-typed reasoning models (e.g. OpenRouter's claude-sonnet-5, type==='effort', no toggle) were misclassified as non-reasoning and got no floor — retries silently bumped raw×4 (2048→8192) instead of applying the floor on the first attempt, masking a hard failure as a benign 'recovered' warning. Fixed via a workerModelReasons predicate that also recognizes type:'effort'. Verify via lore.log after restart: clean distills with zero finish\_reason=length truncation, or retries starting from 16384 rather than 2048/4096.
...
<!-- lore:019f84dc-7598-7218-b0d2-8ced5cd3f7dc -->
* **workerThinkingOnByDefault misses EFFORT-typed reasoning models — curator truncation**: Trap: \`workerThinkingOnByDefault\` (packages/gateway/src/llm-adapter.ts) only recognizes toggle-style reasoning support. EFFORT-typed models (models.dev \`reasoning\_options:\[{effort}]\`, no toggle, e.g. OpenRouter's \`anthropic/claude-sonnet-5\`) fall through and return false, so the reasoning floor never applies. The curator's small budget (2048) gets fully consumed by hidden reasoning tokens, producing \`finish\_reason:"length"\` with empty output on the first attempt — this survived PR #1418 as a production regression. Fix: raised \`DEFAULT\_REASONING\_MODEL\_BUDGET\` 8192→16384 (llm-adapter.ts:469), lifting the effective no-effort floor from 16384→24576 (with \`THINKING\_OUTPUT\_HEADROOM\`=8192) so the curator's heavy reasoning pass lands on attempt 1. Code comment rationale: "a floor, never a charge" — raising it only changes \`max\_completion\_tokens\` sent to the provider, not billed tokens (real cost: provider-side rate-limit pre-reservation for background workers). Confirmed load-bearing via mutation test: reverting to 8192 turns 5 tests red.
...
<!-- lore:019f84c6-0e20-77c1-be99-997cc0e576fc -->
* **Always require adversarial code review and root-cause fixes before merging PRs**: The user consistently requires a rigorous, autonomous PR workflow: implement a fix, run adversarial/mutation-tested review to find real (non-vacuous) bugs, verify CI is fully green (no NEUTRAL/SKIPPED ambiguity), address all findings (fixing blockers inline, filing follow-up issues for non-blocking or latent concerns), and only then merge. When a bug is found, prefer root-cause fixes over surface patches (e.g., reclassifying a 'salience' bug as a 'selection' bug, or reverting a shipped-but-dead-code feature rather than patching its heuristic). The user also expects the assistant to self-direct: continue autonomously through this review→fix→file-issues→merge cycle without stopping to ask permission, check for other in-flight work to avoid collisions, and proactively identify/prioritize the next highest-impact remaining work item after each merge.
...
<!-- lore:019f84cd-f390-7b91-8f8e-fd9b6c1f3774 -->
* **Don't declare a production fix 'confirmed working' from one clean retry**: When verifying a deployed fix via logs, Burak Yigit Kaya pushes back on declaring it 'working' from a single clean data point (e.g. one successful retry after restart) — asked "you sure you don't wanna check again?" prompting a full re-check: scan the ENTIRE log window since restart for all truncation/retry events, confirm the exact count, check for absence of new escalation/degradation log lines, and explicitly flag residual caveats (e.g. first-attempt truncation still occurring, relying on retry to recover) rather than presenting a clean verdict. Extends the existing mutation-testing discipline "never trust the fix without seeing it fail" \[\[019f7cb9-2e4b-7706-86ad-67e04fbeab16]] to production log-based verification, not just pre-merge testing.

<!-- lore:019f84db-07d6-7f8f-bd93-cdadc8074362 -->
* **Enforce mutation-testing verification before merging any bug-fix**: After implementing a fix (especially budget/floor/retry logic in llm-adapter.ts), always verify tests are load-bearing via mutation testing: revert the fix, confirm the expected tests go RED (fail), then restore the fix and confirm the tree is clean/tests pass again. Do this for every distinct invariant introduced (e.g., gating conditions, clamp bounds, floor values). Only after mutation verification, typecheck, lint, and full test suite all pass should the assistant commit, push a branch, open a PR referencing prior related PRs/issues with production evidence, and launch an adversarial review as a background agent — merging only when that review is clean. Never skip straight from implementation to commit without proving tests actually catch regressions of the specific fix.

<!-- lore:019f84c4-2c23-70c1-9abc-f55ee52f17bb -->
* **Follow the established git workflow (branch, PR, review)**: Behavioral pattern detected across 128 sessions (action: enforced-workflow). The user consistently demonstrates this behavior.
...
<!-- lore:019f84cd-f3bf-76e8-bee0-d8d7f410e0a4 -->
* **Launch adversarial PR review as a background task, resume on completion — don't poll**: When requesting adversarial review before merging a PR \[\[019f848b-dbf5-743e-983f-b42f614fd863]], launch it as a background agent task and let the assistant resume automatically when it completes, rather than polling for status. Reiterated as a standing instruction across sessions. Mirrors the pty \<pty\_exited> no-poll rule \[\[019f81c5-dde6-7ffd-a96f-34e3ccde0a5e]] but applies specifically to background subagent review tasks, not just shell processes.

<!-- lore:019f84d8-dc34-777c-b333-6f2adf523732 -->
* **Maintain jj-worktree hygiene: rebase onto latest main and rebuild before evaluation runs**: User consistently works across multiple isolated jj worktrees/workspaces for parallel PR/eval work on the loreai monorepo, and expects the assistant to: (1) keep each worktree rebased onto the true latest main before using it, especially the eval-live build worktree used to produce gateway bundles for eval matrix runs; (2) detect and resolve rebase conflicts by checking whether stale uncommitted changes are already superseded by landed main commits, discarding them if so rather than merging; (3) verify bundle freshness (timestamps/size) and confirm exact main tip commit before running evals or reviews; (4) when a worktree directory is unexpectedly missing, check registered jj workspaces, confirm no committed work is lost, then forget/recreate cleanly; (5) prefer reusing existing extraction/helper functions (e.g., extractFilePath/extractCommand) instead of duplicating logic when building related features across worktrees.
...
<!-- lore:019f84d7-57d4-7bba-b3c9-354237a536e4 -->
